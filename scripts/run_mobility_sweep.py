...
# ---------------------------------------------------
# Payload mobility ranges
#
# range = [-mobility, mobility]
#
# Higher mobility => payload can slide farther
# ---------------------------------------------------
...
def run_trial(payload_mobility, controller_entry):
    ...
    log_path = (
        LOG_DIR /
        f"{controller_name}_mobility_{payload_mobility:.2f}.csv"
    )
    ...
    summary = {
        ...
        "rms_tracking_error":
            float(np.sqrt(np.mean(tracking_errors**2))),

        "max_tracking_error":
            float(np.max(tracking_errors)),

        "mean_control_effort":
            float(np.mean(control_efforts)),

        "max_control_effort":
            float(np.max(control_efforts)),

        "max_payload_disp_base_x":
            float(np.max(np.abs(payload_disps))),
    ...
    print(
        f"mobility={payload_mobility:.2f} m | "
...
def main():
    ...
    for payload_mobility in PAYLOAD_MOBILITIES:
        config = copy.deepcopy(CONFIG)

        config["slide_range_min"] = -payload_mobility
        config["slide_range_max"] = payload_mobility
    ...
    print(f"\nSaved mobility sweep summary to: {SUMMARY_PATH}")
